                    ...
                    <ul class="nav flex-column right-user-buttons ">
                        ...
                        <?php if($this->db->get_where("member", array("member_id" => $this->session->userdata('member_id')))->row()->is_closed == 'yes'){ echo " "; }else{?>
                            <li class="listing-hover">
                                ...
                                ?>
                                <?php if($interest_text == translate('pending')) {?>
                                    <button type="button" class="btn btn-sm btn-primary pt-0 pb-0" id="listing_accept_<?=$member->member_id?>" onclick="confirm_accept(<?=$member->member_id?>)"><?php echo translate('accept')?></button>
                                    <button type="button" class="btn btn-sm btn-danger pt-0 pb-0" id="listing_reject_<?=$member->member_id?>" onclick="confirm_reject(<?=$member->member_id?>)"><?php echo translate('reject')?></button>
                                <?php } elseif($interest_text == translate('accepted')) { ?>
                                    <i class="fa fa-check-circle"></i><?php echo translate('you_have_accepted_the_interest')?>
                               <?php }elseif($interest_text == translate('rejected')) { ?>
                               <i class="fa fa-check-circle"></i><?php echo translate('you_have_rejected_the_interest')?>
						...
						<li class="listing-hover">
						 ...
						 <?php
						   ...
						   <a <?php if ($message_onclick == 1 && $write_message == true){?>onclick="return confirm_message(<?=$member->member_id?>)"<?php }?> href="<?php if ($message_onclick == 0 && $write_message == true) { echo base_url().'home/profile/messaging-list'; }else{ echo 'javascript:void(0);'; } ?>" id="listing_message_a_<?=$member->member_id?>"
												 class="<?php if ($message_onclick == 0 || $write_message == false) { echo 'disabled-messaging'; } ?>">
						       
										 <?php /*<a <?php if ($message_onclick == 1){?>onclick="return confirm_message(<?=$member->member_id?>)"<?php }?> href="<?=base_url()?>home/profile/messaging-list" id="message_a_<?=$member->member_id?>">*/?>
